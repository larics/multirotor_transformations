...
{
	ROS_INFO("Hello from override constructor");
}
...
bool RcOverride::overrideOFFCallback(
...
bool RcOverride::overrideONCallback(
			std_srvs::Empty::Request& request,
			std_srvs::Empty::Response& response)
{
...
void RcOverride::publishOverrideMessage()
{
	...
	std::copy(
			override_indices_.begin(),
			override_indices_.end(),
			newMsg.channels.elems);
...
void RcOverride::setOverrideIndex(int index)
{
	override_index_ = index;
}

void RcOverride::setOverrideValue(int value)
{
